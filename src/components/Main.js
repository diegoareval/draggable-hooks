   ...
   const [state, setState] = useState(initialData)
    ...
    const onDragEnd = ({ destination, source, draggableId }) => {
        ...
        const start = source.droppableId === 'products' ?
                state.productsColumn[source.droppableId] : 
                state.days[source.droppableId];
        ...
        if(source.droppableId === 'products'){
            const newpIDs = state.productsColumn.products.productIds;
        ...
        source.droppableId === 'products' ? 
        state.products[draggableId].price : 0;

        const newState = {
            ...state,
            totalPrice: parseInt(currentItemPrice) + parseInt(state.totalPrice),
            days: {
                ...state.days,
            ...
            <BodyBlock>
                <DragDropContext onDragEnd={onDragEnd}>
                    <WeekDaysContainer>
                        ...
                        <WeekDaysBlock>
                            {state.daysOrder.map((dayId) => {
                                const day = state.days[dayId];
                                const products = day.productIds.map(productId => state.products[productId]);
                    ...
                    {state.productsColumn.products.productIds.length !== 0 ? 
                        (<ProductsBlock>
                            <Title>{state.productsColumn.products.title}</Title>
                            <Droppable droppableId={state.productsColumn.products.id}>
                                {(provided) => (
                                    <ProductList
                                        ref={provided.innerRef}
                                        {...provided.droppableProps}
                                    >
                                        {state.productsColumn.products.productIds.map((product, index) => 
                                            <Product key={product} product={state.products[product]} index={index} />
                                        )}
